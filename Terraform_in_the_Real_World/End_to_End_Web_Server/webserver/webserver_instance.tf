...
resource "aws_security_group" "tilt_webservers"{
  tags = {
    Name = "${var.ENVIRONMENT}-tilt-webservers"
  }
  
  name          = "${var.ENVIRONMENT}-tilt-webservers"
  description   = "Created for Tilt"
  vpc_id        = var.vpc_id
...
#Resource key pair
resource "aws_key_pair" "tilt_key" {
  key_name      = "tilt_key"
...
resource "aws_launch_configuration" "launch_config_webserver" {
  ...
  security_groups = [aws_security_group.tilt_webservers.id]
  key_name = aws_key_pair.tilt_key.key_name
...
resource "aws_autoscaling_group" "tilt_webserver" {
  name                      = "tilt_WebServers"
  max_size                  = 2
  min_size                  = 1
...
output "load_balancer_output" {
  value = aws_lb.tilt-load-balancer.dns_name
}
